...
public class Utils {
     ...
     * @return {@code true} if the app has read and write access to external storage, otherwise {@code false}.
     */
    public static boolean hasExternalStorageRwPermission(@NonNull Context context) {
     ...
     * @param requestCode Application specific request code to match with a result reported to {@link ActivityCompat.OnRequestPermissionsResultCallback#onRequestPermissionsResult(int,
     *                    String[], int[])}.
    ...
    /**
     * Licensed under the Apache License, Version 2.0, from <a href="https://github.com/consp1racy/material-navigation-drawer/blob/master/navigation-drawer/src/main/java/net/xpece/material/navigationdrawer/NavigationDrawerUtils.java">source</a>
     */
    ...
    public static void hideKeyboard(final @NonNull View view) {
        ...
        // If the window token is null, keyboard hiding will only work on some devices. Samsung devices are known to require a window token for that action.
        if (windowToken == null) {
     ...
     * @param defaultColorResource Default color resource to use if attribute is not present in the theme.
     * @return color resource from theme
     */
    @ColorInt
    public static int getThemeColor(@NonNull final Context context, @AttrRes int attribute, @ColorRes int defaultColorResource) {
